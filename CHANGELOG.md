...
- **Friend tiles wear their mood on the edge too.** The tiles' content had colour; their edge was the same grey hairline on every one, which left the strip looking flat. A tile in a mood now has a 2 dp edge in its colours: fire, gold for a crown, green for steam and yellow for the charge turn slowly round the tile - a gradient rotating behind the content, so two highlights chase each other along the edge, on the native driver - and the resting tiles, the cobwebs and the never-trained get a still edge in their own colour, strong at one corner and fading at the other. The charge's edge slows as the charge runs down. A tile with nothing to say (planned today) keeps the hairline. The content is inset by the edge's width and otherwise unchanged (`TileBorder`).
...
- **Holding a badge opens the type sheet.** Four rows with the chosen one filled. Note and Delete sit in the corner: Note opens the set's note in the sheet, and is tinted when there already is one, so an empty field no longer pushes the types down on every set. Delete asks nothing: the set leaves the list at once, and "Set 1 deleted - Undo" holds it back from the database for four seconds. Leaving the workout or folding the card ends the undo and the delete goes through.
- **Two or more warm-ups fold into one row**, showing the last of them with a stacked badge - by hand at any time, from the strip above them, done or not, and by themselves 450 ms after the last one is ticked. Tapping the row opens them again, and opened by hand they then stay open for the session. Taking a tick back, or turning another set into a warm-up, opens them straight away.
- **A warm-up starts empty.** A new set copies the numbers of the set above it, so a set turned into a warm-up used to carry working weight. Turning an unticked set into a warm-up now empties its reps and weight - a ticked one keeps what was lifted - and a new set never copies from a warm-up, so the first working set after two warm-ups starts empty rather than at warm-up weight. Whatever is typed into a warm-up stays.
...
- **A resting friend tile has a background of its own.** A tile with nothing on today - no photo, no music, grey ring - was a grey card with a grey icon, the same for everybody. It now carries how long it has been since that person trained: a wash of colour from the corner that follows the days one by one - coral today, pink at two, blue at five and grey by nine, each day in between its own blend. The scale has colours of its own (`heatHot` and on in `colors.js`), clear of the orange, green and yellow the strip uses for live, done and planned and of the purple the music band draws on the same tile - with the number of days itself huge and faint behind the text, cropped by the tile. Someone who has never trained gets "NY" rather than a zero, which would read as today. Trained within the week, the wash breathes, faster the fresher it is. It stops off screen, with the app in the background, and under reduced motion, like every other loop in the strip.
- **A friend tile has a mood, and it follows where they are in their week.** Training right now, the tile is on a fire: a warm glow wells up from the bottom and flickers, sparks rise off it, and flames flicker round the avatar in place of the live pulse. Done for the day, it is still steaming: a mist breathing along the bottom and soft puffs welling up out of it, swelling and thinning as they rise. Trained in the last five days, it is charged and ready to go again: a wide, soft glow pulses steadily behind the avatar, and thin yellow bolts strike here and there around it - never on the face - the way current moves: on at once, a stutter of flashes over a fifth of a second, gone. Seven of them every few seconds for the first two days, five for the next two, three on the fifth day and only now and then. A month or more gone, cobwebs (below). Nobody who has never trained gathers any: there was nothing to leave. The embers and the steam share one glow (`TileGlow`), since the soft glow under the embers is what the steam was asked to feel like; drawn steam wisps read as rigid, and crackling arcs round the edge and then drifting motes read as nervous and as not ready. `buildTileMood` decides, `src/Utils/tileMoodGeometry.js` lays the moving parts out, and `npm run test:friends-wallpaper` pins both.
- **A record today crowns the tile.** Somebody who set a personal record in the workout they finished today still steams, and wears a gold crown tilted on the top of their avatar - placed so both its bottom corners rest exactly on the ring, worked out from the ring's size (`placeCrownOnRing`) rather than nudged by eye - with a ruby for each record - two at the least, three at the most. The rubies glow and glint one after another, and a shine runs across the gold now and then. Somebody still training gets no crown until the workout is over. Friends' sets are private, so the count comes from `workout_record_counts`, a function that hands out how many records a workout holds - the number only, never the sets - and only for a workout the workout policy already lets the caller see; any failure, the function not being there yet included, is simply no crown. Your own tile counts from the phone (`countPersonalRecordsOnDate`), so it is right the moment the set is ticked. Needs `20260924090000_a-friend-can-see-a-record-was-set-today.sql` for friends' crowns (run 2026-09-24).
- **The days-since card on Home wears your mood too.** It was the same green box with the same flame whether you trained this morning or a month ago. It now carries the mood your own friends tile does: on a fire with the flame flickering while a workout is running, steaming once one is done today - with the crown in the flame's place if it held a record - charged with a bolt for an icon for five days after, the day's colour from the same scale after that, and cobwebs from a month. The number takes the mood's colour; the layout is untouched and the moods are drawn behind it. The frames are the friend tiles' own: the charge can centre on the number instead of an avatar (`focus="middle"`), the crown can be placed anywhere (`style`), cobwebs shrink to a narrower box so the two top webs do not meet, and the animation gate (`useAnimationsEnabled`) moved to `animationHooks.js` for both to share.
- **A month without training and the tile gathers cobwebs.** Old webs hang in the top corners and the bottom right - uneven spokes, rings of silk sagging under their own weight, some doubled, some torn with a broken thread hanging down, matted silk near the corner - with a loose strand drooping across the top, a small web over the corner of the avatar, and the picture itself dimmed as if it had gathered dust. It moves while the strip is on screen: the webs sway a degree or two on their corners, a spider lets itself down on a thread and climbs back up, swinging as it goes, and specks of dust drift down. The bottom left stays clear because the status line starts there, the spider hangs to the right of the avatar so it never crosses a face, and all of it sits behind the text and outside the layout. Ice was tried first - drawn as lines and then as a generated texture - and read as a diagram and then as a smudge; thin sagging threads are what vectors are good at. The webs are seeded per tile (`src/Utils/cobwebGeometry.js`), and `npm run test:friends-wallpaper` checks they stay in their corners.
