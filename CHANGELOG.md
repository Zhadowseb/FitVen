...
- **A resting friend tile has a background of its own.** A tile with nothing on today - no photo, no music, grey ring - was a grey card with a grey icon, the same for everybody. It now carries how long it has been since that person trained: a wash of colour from the corner that follows the days one by one - coral today, purple at two, blue at five, drained to grey by nine, each day in between its own blend, and none of them the orange, green or yellow the strip already uses for live, done and planned - with the number of days itself huge and faint behind the text, cropped by the tile. Someone who has never trained gets "NY" rather than a zero, which would read as today. Trained within the week, the tile moves: the wash breathes and a slanted shine crosses it, more often the fresher it is, each tile starting a beat after the one before so a row of them shines in turn. It stops off screen, with the app in the background, and under reduced motion, like every other loop in the strip. The layout is untouched. Live, done and planned tiles keep their own colours. Your own tile takes its count from the phone, so it is right before a sync.
