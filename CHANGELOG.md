...
### Fixed
- `content_reports.target_post_id` was declared `uuid`. `social_post.id` is `bigint generated by default as identity` - the one id in this schema that is not a uuid - so Postgres refused the foreign key and the whole migration rolled back on the first attempt to run it. The test now reads the type out of the social-posts migration and checks this one matches, because nothing in the repo had any opinion about it before.
