...
- **A resting friend tile has a background of its own.** A tile with nothing on today - no photo, no music, grey ring - was a grey card with a grey icon, the same for everybody. It now carries how long it has been since that person trained: a wash of colour from the corner, coral within a day, purple for two or three, blue up to a week and drained to grey after that - none of them the orange, green or yellow the strip already uses for live, done and planned - with the number of days itself huge and faint behind the text, cropped by the tile. Someone who has never trained gets "NY" rather than a zero, which would read as today. The layout is untouched. Live, done and planned tiles keep their own colours. Your own tile takes its count from the phone, so it is right before a sync.
