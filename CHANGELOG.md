...
## [1.1.0] - Unreleased
### Added
- **Reporting, and a filter on what can be posted.** Apple's review guideline 1.2 asks four things of an app with user content and Google Play's UGC policy asks the first two. FitVen had one of them. This is the other two; blocking already existed, and the contact address is on the support page.
  - A post that is not yours now has a menu, with **Report post** and **Report this account** in it. Accounts can also be reported from the followers and following lists, next to Block.
  - The report asks for one of eight reasons and an optional note, and offers to block the same person in the same breath - someone who has just reported a person almost always wants to stop seeing them, and a second screen is where the second half goes undone. A failed block does not take the report down with it.
  - **A post reported by two different people is withdrawn from the feed immediately**, by a trigger, and reviewed by hand afterwards. Hiding first is what keeps the response inside a day without anyone being awake for it. The author still sees their own post.
  - Reports are readable only by the person who filed them. The reported person is never told who reported them.
- **A word filter on everything a user can publish**: post titles and notes, display names, bios and usernames. It is enforced by database triggers, not by the app - the client writes to `social_post` directly, so a check that only lives in JavaScript is a check anybody can skip. The app calls `is_text_allowed` first so the refusal arrives as a sentence under the field instead of a failed write, and that function answers yes or no without ever naming the word it objected to.
- The term list lives in `private.blocked_terms` so it can be extended from the Supabase dashboard the day something gets past it - no app release, no review wait. It is seeded with a deliberately small set of unambiguous English slurs and sexual terms. It is a floor, not a moderation policy: reporting is what catches everything a word list cannot, which is most of it. Whole words only, because a filter that rejects "Scunthorpe" is a filter people route around; "fag" is left out of the seed because it is an ordinary Danish word.
- `npm run test:content-moderation` checks the parts that can be checked without a database: that every reason the app offers is a reason the check constraint accepts, that both triggers and the hide-on-report trigger exist, that the feed policy respects `hidden_at`, that no policy lets one person read another's reports, that the client never reads the term list, and that reporting is reachable from the content rather than from settings.

### Notes
- **The migration has not been run.** `supabase/migrations/README.md` says so in its ledger. Until `20260912120000_content-moderation.sql` is applied, reporting fails with the setup message and the filter does nothing - so it has to be run before the next store submission, not after.
- Not verified on a device: the phone was disconnected. Nothing here has been seen working against a real database.
- The support page documents how to report and commits to the 24-hour review, which is the half of guideline 1.2 that is words rather than code. `fitven.dk/support/` still returns 404 - the file is on master and Netlify publishes `web/`, so it needs a deploy.
