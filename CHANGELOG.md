...
- **A resting friend tile has a background of its own.** A tile with nothing on today - no photo, no music, grey ring - was a grey card with a grey icon, the same for everybody. It now carries how long it has been since that person trained: a wash of colour from the corner that follows the days one by one - coral today, pink at two, blue at five, grey by nine and frozen by thirty, each day in between its own blend. The scale has colours of its own (`heatHot` and on in `colors.js`), clear of the orange, green and yellow the strip uses for live, done and planned and of the purple the music band draws on the same tile - with the number of days itself huge and faint behind the text, cropped by the tile. Someone who has never trained gets "NY" rather than a zero, which would read as today. Trained within the week, the tile moves: the wash breathes and a slanted shine crosses it, more often the fresher it is, each tile starting a beat after the one before so a row of them shines in turn. It stops off screen, with the app in the background, and under reduced motion, like every other loop in the strip.
- **Fire around the avatar of anyone active, ice around anyone who has gone quiet.** Training now, done today or trained within the last three days, and two rings of flames flicker out of step over a warm glow; the live pulse steps aside for them. A month or more without a workout, and crystals twinkle round a frosted avatar - the ring turns icy and even a photo is tinted over. Both sit around the upper half of the avatar, clear of the name, and outside the layout. Nobody who has never trained is frozen: they were never warm.
- **A month without training and the whole tile freezes over**, like a window pane on a cold morning: frost clouding in thickest at the corners, fine feathered frost along every edge, a crystalline grain that catches the light, a glassy rim and uneven icicles under the top of it. The first attempt drew all of that as SVG lines and read as a diagram of ice, so it is a picture now - grown, not drawn, by `scripts/art/generate-frost-texture.py` (branching ferns, noise-driven haze, blur for the glow), in two variants so two frozen friends side by side do not match, about 180 KB each. It moves while the strip is on screen: the other variant breathes in and out over it so the frost seems to creep and settle, sparkles blink along the rim, a band of light crosses the ice now and then, and a few flakes of snow fall through. All of it sits behind the text, with the middle of the tile left clear for the name and the status line, and outside the layout. In the light theme the frost is tinted ice blue, since white on a white card would not show.
