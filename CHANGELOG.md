...
- Nothing here has run against a device or the live project: the migrations are unapplied, the scraped data is imported only in `--dry-run`, and the Spotify flow has no client id. `npm test` covers the pure rules (best set, upsert, vote status, matching, short names over all 365 folders, tile order, music freshness) and the SQL invariants the app relies on.
