...
- **A month without training and the whole tile freezes over.** It becomes a block of ice: a thick glassy rim, brightest at the top left where the light comes from, frost clouding in from every edge, feathered frost sprays out of each corner and ferns all the way round, loose crystals, and short icicles hanging under the top of the rim. It moves while the strip is on screen - the frost shimmers in two groups out of step, sparkles blink along the rim, a band of light crosses the ice now and then, and a few flakes of snow fall through. All of it is drawn behind the text and outside the layout. The frost is seeded per tile, so each frozen friend has their own and keeps it between renders, and the geometry is plain JavaScript (`src/Utils/frostGeometry.js`) that `npm run test:friends-wallpaper` checks stays inside the tile and clear of the rounded corners. The layout is untouched. Live, done and planned tiles keep their own colours. Your own tile takes its count from the phone, so it is right before a sync.
