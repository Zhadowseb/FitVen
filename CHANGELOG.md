...
- Reporting waits for the options sheet to leave the screen before the dialog opens. Setting both flags in one render puts two native modals up at once, and iOS drops the second without an error, so the button reads as dead. `ThemedBottomSheet` takes an `onDismiss` for this, as `ThemedModal` already did, and `ThemedConfirmModal` takes `confirmDisabled` so **Send report** is greyed out until a reason is picked.
