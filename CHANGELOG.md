...
### Changed
- **The review agents wait to be asked.** `pr-review.yml` no longer runs on every push to a pull request; it runs on a `/qa` comment or a manual dispatch. `qa-gate.yml` posts a check run called "QA agents" as `action_required` on every new head, and the review workflow turns it green once it has actually run - so a pull request cannot be merged before the agents have seen the commit that is being merged. **Make "QA agents" a required check in the branch protection for `master`**; the workflow posts the check, the protection is what makes it a gate, and that setting is yours to make. Overriding it is a deliberate act rather than the default.
...
- The screen is not translated. It is reached by one account, and a key in `locales/` is two languages for everybody who edits it afterwards. The row in the profile that opens it goes through `t()` like the rest of that screen.---## [2.0.0] - Unreleased
### Added
