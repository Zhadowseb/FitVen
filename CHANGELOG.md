...
- **A friend tile has a mood, and it follows where they are in their week.** Training right now, the tile is on a fire: a warm glow wells up from the bottom and flickers, sparks rise off it, and flames flicker round the avatar in place of the live pulse. Done for the day, it is still steaming: a mist breathing along the bottom and soft puffs welling up out of it, swelling and thinning as they rise. Trained in the last three days, it has energy in reserve: a cool glow resting along the bottom and a few motes of light drifting lazily about the tile, six the day after and two by the third, calm on purpose - energy ready, not going off. A month or more gone, cobwebs (below). Nobody who has never trained gathers any: there was nothing to leave. The three glows are one component (`TileGlow`), since the soft glow under the embers is what the other two were asked to feel like; drawn steam wisps and crackling arcs were tried first and read as rigid and as nervous. `buildTileMood` decides, `src/Utils/tileMoodGeometry.js` lays the moving parts out, and `npm run test:friends-wallpaper` pins both.
